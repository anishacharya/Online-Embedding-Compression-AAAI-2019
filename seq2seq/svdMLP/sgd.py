...
class sgd_optimizer:
    def __init__(self, model, model_type):
        ...
        g_x = T.grad(model.cost, model.x)

        self.f_gx = theano.function(inputs=[model.x, model.y],
                                    outputs=[g_x],
                                    on_unused_input='ignore'
                                    )
        self.output = theano.function(inputs=[model.x],
                                    outputs=[model.o],
                                    on_unused_input='ignore'
                                    )
        ...
        elif model.obj =='multi':
            self.f_cost = theano.function(inputs=[model.x, model.y],
                                        outputs=[model.cost,model.accuracy,model.accuracy3, model.accuracy5],
                                        on_unused_input='ignore'
                                        )
    ...
    def update_param(self, flat_delta):
        if self.model_type == 'ResNet':
            ...
            delta = self.flat_to_list(flat_delta)
            # self.p = [W0, b0, U1,V1, P1, b1, ..., Ul, Vl, Pl, bl, Wo, bo ]
            for i in [0,1, -2, -1]:
                ...
                if u % valid_freq == 0:
                    if valid_data is not None:
                        ...
                        if self.obj=='single':
                            ...
                            valid_cost, valid_accuracy, valid_accuracy3, valid_accuracy5= np.mean(results, axis=0)
                        if valid_cost < best[1]:
                            best = u, valid_cost, [i.get_value().copy() for i in self.p]
                    if verbose >= 1 :
                        print_progress(u, num_iter, self.indices_train.epoch, cost, valid_cost, valid_accuracy, valid_accuracy3, valid_accuracy5,  time_cost =  time_used)
...
def print_progress(u, num_iter, epoch, train_cost, valid_cost=None, valid_accuracy=None, valid_accuracy3 = None, valid_accuracy5=None, time_cost = 0):
    if valid_accuracy3 is not None:
        print('update %i/%i, epoch=%i, train_cost=%.5f, time_cost=%.5f, valid_cost=%.5f, valid_accuracy_1=%.5f, valid_accuracy_3=%.5f, valid_accuracy_5=%.5f'
              % (u, num_iter, epoch, train_cost, time_cost, valid_cost, valid_accuracy, valid_accuracy3, valid_accuracy5))
