...
class sgd_optimizer:
    def __init__(self, model, model_type):
        ...
        if self.model_type in ['svdRNN', 'RNN']:
            self.singular_range = theano.function(inputs=[],
                                        outputs=[model.min_singular, model.max_singular],
    ...
    def update_param(self, flat_delta):
        if self.model_type == 'oRNN':
            ...
            self.p[0].set_value(U)
			# update V
            V = np.tril(self.p[1].get_value() + delta[1])
            norms = np.linalg.norm(V, axis=0)
            V = 1. / norms * V
            self.p[1].set_value(V)
			# update Sig, lambda = 0.1
            n_h_ = len(delta[2])
            P = self.p[2].get_value() + delta[2] # m-parametrization
            self.p[2].set_value(P)
			# update else
        ...
        elif self.model_type == 'svdLSTM':
            delta = self.flat_to_list(flat_delta)
            for i in [0,3,6,9]:
                # update U
                U = np.tril(self.p[i].get_value() + delta[i])
                norms = np.linalg.norm(U, axis=0)
                U = 1. / norms * U
                self.p[i].set_value(U)
                # update V
                V = np.tril(self.p[i+1].get_value() + delta[i+1])
                norms = np.linalg.norm(V, axis=0)
                V = 1. / norms * V
                self.p[i+1].set_value(V)
                # update Sig, lambda = 0.1
                n_h_ = len(delta[i+2])
                Sig = (0.9)*(self.p[i+2].get_value() - np.ones(n_h_)) + delta[i+2] + np.ones(n_h_)
                self.p[i+2].set_value(Sig)
            # update else
            for i in range(12, len(self.p)):
                self.p[i].set_value(self.p[i].get_value() + delta[i])
    ...
    def train(self, rng, train_data, valid_data=None, instances_per_batch=None, lambda_=0.001, n_epoch=2000,
        ...
        valid_cost = None
        valid_accuracy = None
        avr_l2_h0 = 0; avr_l1_h0 = 0; avr_inf_h0 =0; avr_neginf_h0 = 0
        ...
        try:
            ...
            print self.indices_train.max_index
            while self.indices_train.epoch <= n_epoch:
                ...
                # Track the vanishing/exloding gradient
                grad_h0 = gradient[-self.shapes[-1][0]:]
                avr_l2_h0 += np.linalg.norm(grad_h0, ord=2)
                avr_l1_h0 += np.linalg.norm(grad_h0, ord=1)
                avr_inf_h0 += np.linalg.norm(grad_h0, ord=inf)
                avr_neginf_h0 += np.linalg.norm(grad_h0, ord=-inf)
                ...
                if u % valid_freq == 0:
                    ...
                    if verbose >= 2 :
                        if self.model_type in ["svdRNN", 'RNN']:
                            cur_min_singular, cur_max_singular = self.singular_range()
                            print("Singular min: %.5f, max: %.5f, L2_H0: %.5f, L1_H0: %.5f, Linf_H0: %.5f, L-inf_H0: %.5f" %
                                    (cur_min_singular, cur_max_singular, avr_l2_h0, avr_l1_h0, avr_inf_h0, avr_neginf_h0))
                            avr_l2_h0 = 0; avr_l1_h0 = 0; avr_inf_h0 =0; avr_neginf_h0 = 0
                        else:
                            print("L2_H0: %.5f, L1_H0: %.5f, Linf_H0: %.5f, L-inf_H0: %.5f" %
                                    (avr_l2_h0, avr_l1_h0, avr_inf_h0, avr_neginf_h0))
                            avr_l2_h0 = 0; avr_l1_h0 = 0; avr_inf_h0 =0; avr_neginf_h0 = 0
