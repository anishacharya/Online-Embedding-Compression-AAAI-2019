...
from Layers import svd_HiddenLayer
...
class svdMLP():
    def __init__(self, rng, n_in, n_out, n_h, n_r, n_layers, margin=0.001,  f_act=leaky_relu, obj='single', dropout_rate=0):
        '''
        :param rng: Numpy RandomState
        :param n_in: Input dimension (int)
        :param n_out: Output dimension (int)
        :param n_h: Hidden dimension (int)
        :param n_r: number of Householder reflectors (int)
        ...
        for i in range(n_layers-1):
            cur_hiddenLayer = svd_HiddenLayer(
                rng=rng,
        ...
        self.optimiser = sgd_optimizer(self, 'svdMLP')
